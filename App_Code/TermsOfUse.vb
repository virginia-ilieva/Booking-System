...
#Region "SQL Statements"
    ...
    Public Sub Read(ByVal theID As Integer)
        ...
        Dim myCommand As SqlCommand = New SqlCommand(String.Format("SELECT * FROM TermsOfUse WHERE ID = {0}", theID), myConnection)
    ...
    Public Sub Save()
        ...
        Dim myReader As SqlDataReader = Nothing
        If Me.ID > 0 Then
            myCommand.CommandText = String.Format("UPDATE TermsOfUse SET BodyContent = '{0}', Created = '{1}' WHERE( ID = {2} )", Me.BodyContent, Convert.ToDateTime(Today).ToString("yyyy-MM-dd"), Me.ID)
        Else
            myCommand.CommandText = String.Format("INSERT INTO TermsOfUse (BodyContent, Created) VALUES ('{0}', '{1}')", Me.BodyContent, Convert.ToDateTime(Today).ToString("yyyy-MM-dd"))
        End If
    ...
    Public Sub Delete()
        Dim myConnection As New SqlConnection
        myConnection.ConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings("DefaultDB").ConnectionString
        Dim myCommand As SqlCommand = New SqlCommand(String.Format("DELETE FROM TermsOfUse WHERE ID = {0}", Me.ID), myConnection)
        Dim myReader As SqlDataReader = Nothing
        myConnection.Open()
        myReader = myCommand.ExecuteReader()
        myConnection.Close()
...
Public Class TermsOfUses : Inherits List(Of TermsOfUse)
    ...
    Public Function Read() As TermsOfUses
        ...
        Using myCommand
            ...
            da.Fill(ds, "TermsOfUse")
            Dim row As DataRow
            For Each row In ds.Tables("TermsOfUse").Rows
                Dim theTermsOfUse As New TermsOfUse
                theTermsOfUse.ID = row("ID")
                theTermsOfUse.BodyContent = row("BodyContent")
                theTermsOfUse.Created = row("Created")
                myTermsOfUses.Add(theTermsOfUse)
            Next
        End Using
        Return myTermsOfUses
    End Function
End Class